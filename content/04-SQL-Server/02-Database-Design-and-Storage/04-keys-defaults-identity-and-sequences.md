...
**When to run the composite demo:** during initial schema design for a time-series or fact table.
**Trigger:** the table's business identity is a small, stable combination (e.g., `(instrument_id, price_date)`).
**Context:** T-SQL DDL against a user database; requires `CREATE TABLE` permission in the target schema; read-only once the table exists.
**Purpose:** prove that a composite clustered primary key uniquely identifies and orders every row without introducing a synthetic surrogate.
...
**When to run:** during initial schema design of a dimension or master-data table that needs a compact foreign-key target and also has a real business key.
**Trigger:** you have chosen `IDENTITY` over `SEQUENCE` because numbering stays inside one table and you do not need pre-insert values.
**Context:** T-SQL DDL in the target database. Requires `CREATE TABLE` permission. State-changing but fully reversible by `DROP TABLE`.
**Purpose:** make the surrogate the clustered PK and enforce business uniqueness separately so `IDENTITY` cannot silently produce duplicate business rows.
...
**When to run:** whenever you need to know the seed, increment, or last value a table's identity column has issued, without actually scanning the table.
**Trigger:** post-DDL verification, pre-migration audit, or reseed planning.
**Context:** read-only T-SQL query against the catalog view [`sys.identity_columns`](https://learn.microsoft.com/sql/relational-databases/system-catalog-views/sys-identity-columns-transact-sql?view=sql-server-ver17).
**Purpose:** confirm that the identity column is declared with the expected seed/increment and read the engine's record of the last value it handed out.
...
**When to run:** in application code or stored procedures that insert one row and need the generated surrogate immediately.
**Trigger:** any INSERT whose generated identity becomes a foreign key or return value.
**Context:** T-SQL executed inside the same batch, stored procedure, or user-defined function as the INSERT. State-changing (the insert itself). Read-only for the retrieval function call.
**Purpose:** obtain the exact identity value generated by the last INSERT in the current scope, immune to triggers that write to other identity tables.
...
**When to run:** whenever an INSERT produces more than one row and the caller needs every generated identity back.
**Trigger:** batch loads, server-side `SELECT INTO` replacements, data copies with SCD-2 versioning.
**Context:** T-SQL INSERT with an `OUTPUT` clause. State-changing (the insert itself); `OUTPUT INSERTED.*` streams a live result set back through the same cursor.
**Purpose:** return every inserted row's generated identity in a single round-trip, without a follow-up SELECT that would re-read the rows.
...
**When to run:** when teaching or validating the "gaps by design" contract, especially before a code review where the proposer expects gap-free numbering.
**Trigger:** any insert that can fail a constraint and retry — the failed attempt still burns an identity value.
**Context:** T-SQL executed against a table with at least one uniqueness constraint. State-changing (consumes an identity value even on failure).
**Purpose:** demonstrate the exact sequence in which SQL Server allocates and discards identity values under a constraint violation, so the reader stops being surprised when production data shows gaps.
...
**When to run:** after large delete or load operations, before migrations, or when diagnosing whether the engine's counter and the physical `MAX()` agree.
**Trigger:** suspicion of drift between `sys.identity_columns.last_value` and `MAX(identity_col)` (e.g., after a manual row deletion that bypassed the table).
**Context:** T-SQL command [`DBCC CHECKIDENT`](https://learn.microsoft.com/sql/t-sql/database-console-commands/dbcc-checkident-transact-sql?view=sql-server-ver17). With `NORESEED` it is read-only; with `RESEED` it writes to the counter. Requires `db_owner` or ownership of the table.
**Purpose:** report the engine's current identity and the actual `MAX()` side by side so the operator can decide whether to leave them alone or reseed.
...
**When to run:** exactly once per session, during a controlled migration or backfill, when specific identity values must be preserved (for example, copying rows from a staging table into its production sibling while keeping the PKs).
**Trigger:** data migration, disaster recovery from an export, or merging partitioned tables.
**Context:** T-SQL `SET IDENTITY_INSERT dbo.table ON;` — session-scoped, only one table at a time. Requires `ALTER` permission on the table. State-changing and trap-prone.
**Purpose:** bypass the automatic generator for one or more explicit `INSERT` statements so specific values can be written into the identity column.
...
**When to run:** on a scheduled interval (monthly or quarterly) across every identity column in the database, as part of a standing capacity check.
**Trigger:** drift report, capacity review, before any project that expects to multiply insert volume on an identity-bearing table.
**Context:** read-only T-SQL query that joins `sys.identity_columns` to `sys.types` to translate the data type into a maximum value, then computes utilization.
**Purpose:** produce a per-identity-column percent-consumed figure so the DBA can see every table approaching exhaustion in one glance.
...
**When to run:** during initial schema design of the schema that owns cross-table numbering.
**Trigger:** a decision to externalise numbering from the tables.
**Context:** T-SQL DDL (`CREATE SEQUENCE`). Requires `CREATE SEQUENCE` permission on the schema. State-changing (creates a schema object; the first `NEXT VALUE FOR` call allocates its first cache block).
**Purpose:** create a reusable number generator with explicit data type, start, step, bounds, non-cycling behaviour, and a cache size tuned to the workload.
...
**When to run:** after creating or altering a sequence, and whenever you need to know its current/last-used values without consuming a number.
**Trigger:** post-DDL audit, pre-migration inventory, or diagnosing an `is_exhausted` condition.
**Context:** read-only query against the catalog view [`sys.sequences`](https://learn.microsoft.com/sql/relational-databases/system-catalog-views/sys-sequences-transact-sql?view=sql-server-ver17).
**Purpose:** read every declared property and the current state of the generator in one row.
...
**When to run:** when you want the sequence to feel like an identity column from the application's perspective — the caller never names the value.
**Trigger:** binding a table's PK to a pre-existing sequence.
**Context:** T-SQL DDL (`CREATE TABLE` with `DEFAULT (NEXT VALUE FOR ...)`). Requires `CREATE TABLE` plus `SELECT` (for `NEXT VALUE FOR`) and `REFERENCES` on the sequence.
**Purpose:** let inserts that omit the PK column draw the next value from the sequence automatically, without giving up the option to supply an explicit value.
...
**When to run:** in procedures that need the value in a variable before the insert, or when you want the retrieval mechanism to be visible at the call site.
**Trigger:** the caller wants to embed the sequence value in a message, a log line, or a return parameter before the row is persisted.
**Context:** T-SQL `INSERT ... VALUES (NEXT VALUE FOR ...)`. State-changing.
**Purpose:** record the exact value the sequence issued for the new row.
...
**When to run:** from a batch process that will issue many INSERTs and wants to reserve its range in one round-trip.
**Trigger:** bulk load, ETL pipeline, or an application that shards surrogate IDs across threads.
**Context:** T-SQL `EXEC sys.sp_sequence_get_range` with four output parameters. Requires `UPDATE` on the sequence or its schema.
**Purpose:** reserve a contiguous range of sequence values without N round-trips, and learn how many times the sequence had to cycle (for cycling sequences) to satisfy the request.
...
**When to run:** when the sequence's counter must jump backwards (rarely correct), or when an initial seed was wrong and the sequence has not yet been consumed in production.
**Trigger:** design error, test-environment reset, or a deliberate restart after a cleanup.
**Context:** T-SQL `ALTER SEQUENCE ... RESTART WITH`. Requires `ALTER` on the sequence. State-changing.
**Purpose:** change `RESTART`, `INCREMENT`, `MINVALUE`, `MAXVALUE`, `CYCLE`, or `CACHE` on an existing sequence without recreating it.
...
**When to run:** during initial schema creation for a table where several columns should auto-populate when the caller omits them.
**Trigger:** new table for logs, audits, events, job control, or any scenario with system-generated columns.
**Context:** T-SQL `CREATE TABLE` with one `CONSTRAINT DF_... DEFAULT (...)` clause per column. Requires `CREATE TABLE` permission. State-changing.
**Purpose:** name every default constraint explicitly so subsequent `ALTER TABLE DROP CONSTRAINT` operations are stable across environments.
...
**When to run:** any time you need to audit, reproduce, or remove a default constraint in code.
**Trigger:** environment diff, post-deployment audit, schema-export script.
**Context:** read-only query against [`sys.default_constraints`](https://learn.microsoft.com/sql/relational-databases/system-catalog-views/sys-default-constraints-transact-sql?view=sql-server-ver17) joined to `sys.columns` for the column name.
**Purpose:** list every default attached to a table, the column it targets, and the exact expression the engine will evaluate.
...
**When to run:** to prove that omitting columns in an INSERT statement causes their defaults to fire.
**Trigger:** writing or reviewing a client that relies on defaults.
**Context:** T-SQL INSERT statements that omit various default-bound columns. State-changing.
**Purpose:** demonstrate that defaults fire only for omitted columns, that explicit values override them, and that all default types (function, literal, JSON string) coexist in the same table.
...
**When to run:** before adopting `NEWID()` as a clustered PK default in any new table.
**Trigger:** a design review where someone proposes `uniqueidentifier DEFAULT NEWID()` as the PK.
**Context:** T-SQL against [`sys.dm_db_index_physical_stats`](https://learn.microsoft.com/sql/relational-databases/system-dynamic-management-views/sys-dm-db-index-physical-stats-transact-sql?view=sql-server-ver17) in `'DETAILED'` mode. Requires `VIEW DATABASE STATE`. Read-only but scans every page of the leaf level, which is expensive on large tables.
**Purpose:** produce a side-by-side fragmentation measurement of two clustered indexes — one keyed by `NEWID()`, one by `NEWSEQUENTIALID()` — after inserting the same number of rows into each.
...
**When to run:** during schema design of a row whose updates must be protected from lost-update bugs without taking long pessimistic locks.
**Trigger:** any scenario where multiple clients read a row, edit it independently, and write back — accounts, inventory, configuration documents.
**Context:** T-SQL `CREATE TABLE` with a single `rowversion` column. The engine stamps and maintains the value automatically. State-changing; one `rowversion` column per table is allowed.
**Purpose:** equip the row with a change token so subsequent UPDATEs can detect whether the row was modified since the client read it.
...
**When to run:** in a client that read the row earlier, cached the `rv` value, and now wants to commit its edit only if no one else has modified the row in between.
**Trigger:** any UPDATE where lost-update bugs are unacceptable.
**Context:** T-SQL `UPDATE` with an additional `AND rv = @snapshot_rv` predicate. State-changing. Requires `UPDATE` on the table.
**Purpose:** use the previously-captured `rv` token as a concurrency check: `@@ROWCOUNT = 1` means the update landed; `@@ROWCOUNT = 0` means someone else updated the row first and the client must re-read and retry.
...
**When to run:** at the start of every incremental sync window.
**Trigger:** a downstream pipeline that needs "all rows changed since the previous sync".
**Context:** read-only T-SQL call to [`MIN_ACTIVE_ROWVERSION()`](https://learn.microsoft.com/sql/t-sql/functions/min-active-rowversion-transact-sql?view=sql-server-ver17) at the start of the window, and `@@DBTS` at the end — both are non-deterministic and not affected by isolation level.
**Purpose:** obtain a high-watermark value that is safe for incremental reads. `MIN_ACTIVE_ROWVERSION` is the lowest `rv` that may still be in an uncommitted transaction; any rv strictly less than it is guaranteed to be committed.
