...
A race condition occurs when two or more processes access shared data concurrently, and the final result depends on the timing of their execution. Unlike [[deadlock-detection-and-prevention|deadlocks]] (where processes get stuck), both processes complete — but the data ends up wrong.
...
The dashboard uses the `WithDeadlockRetryAsync` wrapper to handle the rare case where a SELECT is caught in a [[deadlock-detection-and-prevention|deadlock]] with a pipeline write.
