...
SQL Server Audit tracks all security-relevant events (login attempts, permission changes, data access) to binary audit files (`.sqlaudit`). Required for regulatory compliance (IOSCO/ESMA for benchmark administrators, GDPR Article 30 data access logs). Events are written to disk and can be forwarded to GCP Cloud Logging for centralized monitoring and alerting.
