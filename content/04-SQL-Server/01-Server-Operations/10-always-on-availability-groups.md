...
- **Schema-locks from readers.** If the secondary is serving read-intent workloads, those readers can hold schema stability (`Sch-S`) locks that block the redo thread. Enable [RCSI](https://alp78.github.io/elysium/04-SQL-Server/03-Query-Writing-and-Optimization/blocking-and-locking) on the secondary so readers use row-versioning instead of schema locks.
...
- [backup-types-and-strategy](https://alp78.github.io/elysium/04-SQL-Server/01-Server-Operations/backup-types-and-strategy) — FULL recovery model required for AGs; backup strategy with AG offload
