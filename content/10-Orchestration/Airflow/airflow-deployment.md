...
Docker Compose is the fastest way to run a full Airflow environment locally. The official `docker-compose.yaml` from Apache runs all components in containers, making it easy to reproduce the production environment on a laptop.
...
For teams that need more control than managed services provide, or want to minimize cloud-managed service costs.
