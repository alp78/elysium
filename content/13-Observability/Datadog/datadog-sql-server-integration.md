...
**Full config with custom queries** — see [datadog-custom-queries](/13-Observability/Datadog/datadog-custom-queries) for the complete file including both custom query blocks.
...
| `username` / `password` | `dd_agent` | Read-only login with `VIEW SERVER STATE` — see [datadog-agent-sql-vm](/13-Observability/Datadog/datadog-agent-sql-vm) |
...
- [datadog-agent-sql-vm](/13-Observability/Datadog/datadog-agent-sql-vm) — How the agent is installed and managed
- [datadog-custom-queries](/13-Observability/Datadog/datadog-custom-queries) — Adding custom DMV metric queries to this config
- [datadog-log-management](/13-Observability/Datadog/datadog-log-management) — Separate config for SQL Server errorlog collection
- [datadog-dashboards](/13-Observability/Datadog/datadog-dashboards) — Dashboard widgets using these metrics
