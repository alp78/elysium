...
tags: [sql-server, python, etl, bronze, medallion, pyodbc, ddl, how-to]
...
The bronze layer is the raw data landing zone in the [[medallion-architecture]]. Every table stores data exactly as received from the source — 1:1 with the source [[data-formats-and-serialization|JSON files]] produced by yfinance fetchers. No business logic is applied; transformations happen in [[silver-transforms|silver]].
