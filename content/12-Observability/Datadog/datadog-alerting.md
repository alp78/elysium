...
The monitor triggers immediately when a deadlock occurs and sends an email with remediation steps. It auto-resolves when no new deadlocks are detected in the evaluation window.
...
|----------|---------|---------------|
| P1 (Critical) | Scheduler down — pipeline is completely blocked | Immediate |
