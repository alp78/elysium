...
- **Column-level confidentiality from high-privilege DBAs** — any `sysadmin` or `db_owner` can `SELECT *` and read plaintext. Column-level protection requires [Always Encrypted](https://alp78.github.io/elysium/04-Databases/01-SQL-Server/02-Security-Hardening/03-always-encrypted) or explicit application-layer encryption.
