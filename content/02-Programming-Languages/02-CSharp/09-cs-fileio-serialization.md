---
title: "09 - File I/O and Serialization - C#"
tags: [csharp]
...
Console.WriteLine(Path.Combine(tmpDir, "output", "data.csv"));
...
Console.WriteLine(sw.ToString().TrimEnd());   // In-memory CSV
...
Console.WriteLine(run2!.PipelineId);     // Pipeline
Console.WriteLine(run2.Status);          // Status
Console.WriteLine(run2.ErrorMessage);    // Error
```
...

{
    ...
    Console.WriteLine(root.GetProperty("job_id").GetString());                // Job
    var stats = root.GetProperty("statistics");
    Console.WriteLine(stats.GetProperty("total_rows").GetInt64());              // Rows
    Console.WriteLine(stats.GetProperty("cache_hit").GetBoolean());             // Cache hit
}
```
...
```csharp
{
    ...
    utf8Writer.Flush();
    Console.WriteLine(Encoding.UTF8.GetString(ms.ToArray()));   // Utf8JsonWriter output
}
...
using (var ms = new MemoryStream())
{
    ...
    Console.WriteLine(Encoding.UTF8.GetString(ms.ToArray()).TrimEnd());   // Content
}
```
...
```csharp
{
    ...
    Console.WriteLine(Encoding.UTF8.GetString(uploadStream.ToArray()).TrimEnd());   // Preview
}
```
...
Console.WriteLine(builtString.TrimEnd());   // StringWriter output
```
...
Console.WriteLine(new FileInfo(binFile).Length);   // bytes written
```
...
```csharp
{
    ...
    Console.WriteLine($"  From memory: sensor={brMem.ReadInt32()}, value={brMem.ReadDouble():F1}, alert={brMem.ReadBoolean()}, region={brMem.ReadString()}");
}
```
...
using (var fs = new FileStream(rawFile, FileMode.Create, FileAccess.Write))
{
    ...
    fs.Write(data);
    Console.WriteLine(fs.Length);   // bytes written
}
...
Console.WriteLine(content.TrimEnd().Replace("\n", ", "));   // Read async
...
Console.WriteLine(await File.ReadAllTextAsync(asyncLog));   // Async log
...
Console.WriteLine(BitConverter.ToString(hashBytes));          // Bytes
Console.WriteLine(hex);                                       // Hex
Console.WriteLine(hexLower);                                  // Hex lower
Console.WriteLine(hashBytes.SequenceEqual(backToBytes));      // Roundtrip

// SHA-256 hash displayed as hex (standard format)
var sha256 = SHA256.HashData(Encoding.UTF8.GetBytes("SAP.DE"));
Console.WriteLine(Convert.ToHexString(sha256).ToLower());     // SHA-256 of "SAP.DE"
...
Console.WriteLine(sample);                                                                // Original
Console.WriteLine($"  UTF-8 bytes: [{string.Join(", ", sampleBytes.Select(b => $"0x{b:X2}"))}]");
Console.WriteLine(Convert.ToBase64String(sampleBytes));                                   // Base64
Console.WriteLine(Convert.ToHexString(sampleBytes));                                      // Hex
Console.WriteLine(Uri.EscapeDataString(sample));                                          // URL

Console.WriteLine($"  {"Format",-15} {"Output",-40} {"Size",5}");
...
>
> For large files (hundreds of MB or more), this causes `OutOfMemoryException` or severe GC pressure. The entire string lives on the Large Object Heap.

> [!success] Correct pattern
>
> Use `StreamReader` for line-by-line processing, or `System.IO.Pipelines` for high-throughput scenarios.

> [!warning] Creating `JsonSerializerOptions` per call is expensive
>
> Each instantiation allocates internal caches. In a hot loop, this creates significant GC pressure.

> [!success] Correct pattern
>
> Create once and reuse: `private static readonly JsonSerializerOptions _opts = new() { PropertyNameCaseInsensitive = true };`.

> [!warning] `System.Text.Json` is case-sensitive by default
>
> `{"Name": "Alice"}` won't deserialize into a property named `name` (lowercase) unless case-insensitive mode is enabled.

> [!success] Correct pattern
>
> Set `PropertyNameCaseInsensitive = true` in `JsonSerializerOptions`, or use `[JsonPropertyName("name")]` attributes.

> [!warning] Manual `string.Split(',')` for CSV breaks on quoted fields
>
> `"Smith, Jr.",42` splits into three fields instead of two. Quoted commas are part of the field value.
...
- **Cache `JsonSerializerOptions`** — create once as a `static readonly` field and reuse across calls.
- **Use `File.ReadAllTextAsync` for async code** — prevents blocking the thread pool on I/O.
- **Use `Span<T>` parsing for hot paths** — `Utf8JsonReader` and `ReadOnlySpan<char>` eliminate heap allocations in parsers.
- **Use memory-mapped files for very large files** — random access without loading the entire file into memory.
...
| Problem | Cause | Fix |
|---|---|---|
| `OutOfMemoryException` reading large file | `File.ReadAllText` loads entire content into memory | Use `StreamReader` for line-by-line processing |
| `JsonException: The JSON value could not be converted` | Type mismatch between JSON value and target property | Check JSON structure matches the target record/class |
| JSON deserialization returns null/default values | Property name case mismatch (C# PascalCase vs JSON camelCase) | Set `PropertyNameCaseInsensitive = true` or use `[JsonPropertyName]` |
| `ObjectDisposedException` on stream access | Used stream after its `using` scope ended | Ensure all reads/writes happen within the `using` block |
| CSV field split incorrectly | Manual `Split(',')` on quoted CSV data | Use CsvHelper or a proper CSV library |
| `IOException: The process cannot access the file` | File locked by another process | Close other programs; use `FileShare.Read` for concurrent access |
