...
> - .NET isolates packages per-project via `.csproj`; the real isolation problem is SDK versions, not package versions.
> - A `global.json` in the repo root pins the SDK for all contributors and CI.
>
> **Project creation and dependency management**
> - `dotnet new` scaffolds projects; the `.csproj` is both the environment spec and the dependency manifest — no separate activation step.
> - `dotnet add package` appends a `<PackageReference>` to `.csproj`; `dotnet restore` downloads and caches packages in `~/.nuget/packages/`.
> - Version ranges support exact pins, floating patches (`2.1.*`), and bracket notation; exact pins are mandatory in production.
>
> **Deterministic builds**
> - `packages.lock.json` is opt-in (`RestorePackagesWithLockFile`); commit it and enforce it in CI with `dotnet restore --locked-mode`.
> - Central Package Management consolidates versions across all solution projects in `Directory.Packages.props`.
>
> **Environment inspection**
> - `dotnet --info` reports the active SDK, all installed SDKs and runtimes, OS, and RID.
> - `dotnet list package --include-transitive` exposes the full dependency tree; `--vulnerable` audits for CVEs.
>
> **Docker multi-stage builds**
> - Build stage uses the SDK image (~1.5 GB); runtime stage uses `runtime` or `aspnet` (~200 MB).
> - Copy `.csproj` first, restore, then copy source to preserve Docker layer cache.
>
> **CI/CD and GCP deployment**
> - `actions/setup-dotnet@v4` installs the SDK; pass `global-json-file: global.json` to auto-derive the version.
> - Cloud Run is the natural GCP fit for .NET containers; Cloud Functions support is limited and cold-start-heavy.
> - Terraform deploys infrastructure pointing at Docker images; secrets are injected at runtime via environment variables or Secret Manager.
>
> **Anti-patterns**
> - Root causes of "works on my machine": missing `global.json`, floating versions without lock files, undocumented environment variables.
> - SDK image in production, `COPY . .` before restore, and floating `*` version ranges are the most frequent Docker/NuGet mistakes.

> [!note]- Glossary
>
> **`.csproj`**
> - XML project file declaring the target framework, package references, and build properties; the primary dependency contract for a .NET project.
> - `dotnet restore` reads it to fetch and pin all NuGet packages; `dotnet add package` is the safe way to modify it, avoiding malformed XML.
>
> > [!warning] Editing `.csproj` by hand
> >
> > Manual edits without understanding XML escaping can silently corrupt the project file. Always use `dotnet add package` and `dotnet remove package` for dependency changes.
>
> > ---
>
> **`dotnet restore`**
> - Downloads all NuGet packages declared in `.csproj` into `~/.nuget/packages/` and writes a `project.assets.json` lockfile; idempotent.
> - Must run before `dotnet build`; CI typically runs it explicitly as a separate step with `--locked-mode` for cache and visibility.
>
> > [!info] Implicit restore in development
> >
> > `dotnet build` implicitly runs restore in development mode. Many CI templates disable this with `--no-restore` to keep steps isolated and speeds incremental builds.
>
> > ---
>
> **NuGet**
> - The package manager for .NET, hosting versioned and signed packages on `nuget.org` with transitive dependency resolution via `PackageReference`.
> - NuGet package versions are independent of the .NET SDK version — `Newtonsoft.Json 13.0.3` works on any .NET 6/7/8/9 target.
>
> > [!tip] Global package cache is shared
> >
> > Unlike Python's per-venv `site-packages`, NuGet caches packages globally at `~/.nuget/packages/`. Two projects referencing `Dapper 2.1.28` share one cached copy. `dotnet nuget locals all --clear` affects every project on the machine.
>
> > ---
>
> **`global.json`**
> - A JSON file at the repository root that pins the .NET SDK version for all projects in the directory tree via a `rollForward` policy.
> - Must be placed at the repo root or a parent directory; placing it inside a project subfolder has no effect on `dotnet` invocations above it.
>
> > [!warning] Stale `global.json` in a parent directory
> >
> > .NET searches up the directory tree for `global.json` and uses the first file found. A `global.json` in a parent or home directory silently overrides the project-level one, causing "wrong SDK" failures.
>
> > ---
>
> **`packages.lock.json`**
> - An optional lockfile generated by `dotnet restore` recording the exact resolved version and content hash of every transitive dependency.
> - Requires explicit opt-in (`RestorePackagesWithLockFile=true`); commit it and enforce it with `dotnet restore --locked-mode` in CI to make builds fully deterministic.
>
> > [!danger] Advisory-only lockfile
> >
> > Committing `packages.lock.json` without setting `RestoreLockedMode=true` in CI leaves the lockfile advisory only. `dotnet restore` will silently update it, producing different transitive versions than what was tested locally.
>
> > ---
>
> **Multi-stage Dockerfile**
> - A Dockerfile with multiple `FROM` stages where the final image contains only runtime artifacts — no SDK, no source code, no intermediate build files.
> - Build stage uses `mcr.microsoft.com/dotnet/sdk` (~1.5 GB); runtime stage uses `mcr.microsoft.com/dotnet/runtime` or `aspnet` (~200 MB), reducing the final image by ~85%.
>
> > [!danger] Single-stage SDK image in production
> >
> > Using `FROM mcr.microsoft.com/dotnet/sdk` as the final stage ships compilers, NuGet client, and source into production — roughly 700 MB of unnecessary attack surface. Always use a multi-stage build.
>
> > ---
>
> **`dotnet publish`**
> - Compiles the project and copies all required files (compiled DLLs, dependencies, config) to a publish directory; output is what enters the Docker runtime stage or server.
> - Framework-dependent publish (default) produces ~5 MB output requiring the runtime on the target; `--self-contained` bundles the runtime (~80 MB extra) for targets without .NET installed.
>
> > [!warning] `dotnet build` output is not deployment-ready
> >
> > `dotnet build` output includes `.pdb` files, test projects, and intermediate artifacts. Always use `dotnet publish -c Release` to produce the deployment artifact.
>
> > ---
>
> **`dotnet add package`**
> - Adds a `<PackageReference>` entry to the `.csproj` file and immediately runs `dotnet restore`; the safe way to add or upgrade dependencies.
> - Validates the package exists on the configured NuGet feed and resolves the version before writing to `.csproj`; running `dotnet add package <name>` again with `--version` overwrites the existing pin.
>
> > [!tip] Find the latest version before pinning
> >
> > Run `dotnet add package Dapper` once without `--version` to resolve the latest stable. Copy the resolved version from `dotnet list package` into the explicit `--version` flag for the pinned production reference.
>
> > ---
>
> **Central Package Management (CPM)**
> - A .NET 7+ feature using `Directory.Packages.props` to declare package versions once for all projects in the solution, eliminating per-project version drift.
> - Activated by `<ManagePackageVersionsCentrally>true</ManagePackageVersionsCentrally>`; per-project `Version` attributes on `<PackageReference>` are then disallowed.
>
> > [!warning] Mixing CPM with per-project versions
> >
> > When `ManagePackageVersionsCentrally=true`, adding a `Version` attribute to an individual `<PackageReference>` causes a build error. Remove all version attributes from individual `.csproj` files once CPM is enabled.
>
> > ---
>
> **Artifact Registry (NuGet feed)**
> - GCP's managed repository service hosting private NuGet packages alongside Docker images and Maven artifacts; authenticated via Workload Identity or service account key.
> - Configured in `nuget.config` under `<packageSources>`; credentials must be supplied via environment variables or credential providers — never hardcoded in source control.
>
> > [!danger] Hardcoded feed credentials in `nuget.config`
> >
> > Storing the PAT or service account key directly in `nuget.config` and committing it exposes credentials to everyone with repo access. Use `%NUGET_PAT%` environment variable substitution or `dotnet nuget update source` per machine.
>
> > ---
>
> **`actions/setup-dotnet@v4`**
> - The official GitHub Actions action installing a specific .NET SDK version on the CI runner; pass `global-json-file: global.json` to derive the version from the committed `global.json` automatically.
> - Installing without a version pin lets the action pick the latest SDK at run time, breaking reproducibility when a new SDK band is released mid-sprint.
>
> > [!tip] Align `setup-dotnet` with `global.json`
> >
> > Set `dotnet-version: '8.0.x'` in `actions/setup-dotnet@v4` and `"rollForward": "latestPatch"` in `global.json`. Both resolve to the same SDK band, eliminating version conflicts without removing `global.json`.
>
> > ---
>
> **Self-contained publish**
> - A `dotnet publish` mode bundling the .NET runtime alongside the application, producing an executable that runs without a pre-installed SDK on the target machine.
> - Increases binary size by ~60–80 MB; intended for bare-metal targets or locked-down VMs — not Docker containers, where the `mcr.microsoft.com/dotnet/aspnet` base image is smaller and receives Microsoft security patches.
>
> > [!warning] Self-contained publish in Docker
> >
> > Using `--self-contained` in a Docker image duplicates the runtime already present in the base image, adding ~80 MB of unnecessary size. Use framework-dependent publish with the appropriate MCR base image for all container targets.
...
|---|---|---|---|
| **`.csproj`** | An XML project file that declares the target framework, package references, and build properties for a .NET project | The primary dependency contract — `dotnet restore` reads it to fetch and pin all NuGet packages | Editing `.csproj` manually without understanding XML escaping — use `dotnet add package` to avoid introducing malformed XML |
| **`dotnet restore`** | Downloads all NuGet packages declared in `.csproj` into the per-user global cache and writes a `project.assets.json` lockfile | Must be run before `dotnet build`; CI typically runs it explicitly as a separate step for caching | Assuming `dotnet build` automatically restores — it does in development, but many CI templates disable implicit restore (`--no-restore`) for faster incremental builds |
| **NuGet** | The package manager for .NET that hosts packages on nuget.org and enterprise feeds | Provides versioned, signed packages with transitive dependency resolution via `PackageReference` | Confusing NuGet package versions with .NET SDK versions — `Newtonsoft.Json 13.0.3` works on any .NET 6/7/8/9 target; the package and runtime versions are independent |
| **`global.json`** | A JSON file at the repository root that pins the .NET SDK version used for all projects in that directory tree | Ensures all developers and CI agents use the exact same SDK, preventing `CS8xxx` language feature mismatches | Placing `global.json` inside a project subfolder — it must be at the repository root or a parent directory to affect `dotnet` invocations below it |
| **`packages.lock.json`** | An optional lockfile generated by `dotnet restore` that records the exact resolved version of every transitive dependency | Enables fully deterministic builds — `dotnet restore --locked-mode` fails if the lockfile is out of date | Checking in `packages.lock.json` but not setting `RestoreLockedMode=true` in CI — the lockfile is then advisory only and `dotnet restore` may still upgrade transients |
| **Multi-stage Dockerfile** | A Dockerfile with multiple `FROM` stages where the final image only contains the runtime artifacts, not the SDK or source code | Separates the build environment (SDK, test tools, NuGet cache) from the runtime image (ASP.NET Core runtime, compiled DLLs only) | Using a single `FROM mcr.microsoft.com/dotnet/sdk` image for the final layer — the SDK image is ~700 MB; the runtime image is ~220 MB |
| **`dotnet publish`** | Compiles the project, resolves self-contained or framework-dependent output, and copies all required files to a publish directory | The output of `dotnet publish` is what gets copied into the Docker runtime stage or deployed to a server | Running `dotnet build` in the Dockerfile instead of `dotnet publish` — `build` output includes `.pdb` files, test projects, and intermediate artifacts not needed at runtime |
| **`dotnet add package`** | Adds a `<PackageReference>` entry to the `.csproj` file and immediately runs `dotnet restore` | The safe way to add dependencies — it validates the package exists on the configured NuGet feed and resolves the version | Editing `.csproj` directly and forgetting to run `dotnet restore` — the project references the package but the assets file is stale and the build fails |
| **Central Package Management (CPM)** | A .NET 7+ feature using a `Directory.Packages.props` file to declare package versions once for all projects in the solution | Prevents version drift when 10 projects all reference `Dapper` at potentially different versions | Mixing CPM (`ManagePackageVersionsCentrally=true`) with per-project `Version` attributes on `PackageReference` — CPM disables per-project versioning entirely |
| **Artifact Registry (NuGet feed)** | GCP's managed repository service that can host private NuGet packages alongside Docker images and Maven artifacts | Used for internal .NET libraries not publishable to nuget.org; authenticated via Workload Identity or service account key | Hardcoding the Artifact Registry URL and credentials in `NuGet.Config` in source control — use environment variables or `dotnet nuget add source` with credential providers |
| **`actions/setup-dotnet@v4`** | The official GitHub Actions action that installs a specific .NET SDK version on the CI runner | Used in CI to ensure the build uses the same SDK version as `global.json` — pass `global-json-file: global.json` to derive the version automatically | Installing the SDK without specifying a version, letting the action pick the latest — this breaks reproducibility when a new SDK is released mid-sprint |
| **Self-contained publish** | A `dotnet publish` mode that bundles the .NET runtime alongside the application, producing an executable that runs without a pre-installed SDK | Used for single-binary deployment on machines where the runtime is not installed; increases binary size significantly (~60 MB overhead) | Using self-contained publish for Docker deployments — Docker images should use the `mcr.microsoft.com/dotnet/aspnet` base image (runtime-only), which is smaller and receives Microsoft security patches |

### What this note covers

- **Why Environments Exist** — SDK version isolation problem and how .NET differs from Python
- **`dotnet new`** — project and solution scaffolding patterns
- **`dotnet add package`** — installing NuGet packages, version syntax
- **Pinning and Locking Dependencies** — `packages.lock.json`, `--locked-mode`, Central Package Management
- **`dotnet add/remove`** — upgrading, listing, removing packages
- **`dotnet --info`** — inspecting the installed SDK and runtime
- **The `.csproj` lifecycle** — from creation through publication and multi-targeting
- **`global.json`** — pinning the SDK version per repository
- **Dockerfile multi-stage** — build and deploy .NET in containers
- **GitHub Actions** — CI/CD for .NET environments
- **GCP deployment** — Cloud Run, App Engine, Cloud Functions for .NET
...
flowchart TD
    A{"Web API or<br/>gRPC service?"} -->|Yes| B["Cloud Run<br/>(aspnet image)"]
    A -->|No| C{"Long-running<br/>background job?"}
    C -->|Yes| D{"Needs persistent<br/>state or GPU?"}
    D -->|Yes| E["Compute Engine<br/>(VM)"]
    D -->|No| F["Cloud Run Jobs<br/>(runtime image)"]
    C -->|No| G{"Lightweight<br/>event handler?"}
    G -->|Yes| H["Cloud Functions<br/>(limited .NET support)"]
    G -->|No| F
```
...
- [Functional Pipeline](https://alp78.github.io/elysium/02-Programming-Languages/02-CSharp/25-cs-functional-pipeline) — Production pipeline using .csproj + NuGet
...
> [!warning] Committing `packages.lock.json` without enabling `RestoreLockedMode` in CI makes the lockfile advisory only
> If `<RestoreLockedMode>true</RestoreLockedMode>` is not set in the `.csproj` or passed as `--locked-mode` to `dotnet restore`, the lockfile is generated on commit but never enforced. A CI build can silently upgrade a transitive dependency while the lockfile sits unchanged.

> [!success] Correct pattern
> Either set `<RestoreLockedMode>true</RestoreLockedMode>` in the `.csproj` (or `Directory.Build.props`) and commit the lockfile, or do not commit the lockfile at all. Half-committed lockfiles are worse than none — they create false confidence.

> [!warning] Using a single-stage Dockerfile with the SDK image produces a ~700 MB production image
> The .NET SDK image (`mcr.microsoft.com/dotnet/sdk`) includes the full compiler toolchain, NuGet client, and intermediate build artifacts. Shipping it to production multiplies image pull time, storage cost, and attack surface.

> [!success] Correct pattern
> Use a two-stage Dockerfile: `FROM mcr.microsoft.com/dotnet/sdk:9.0 AS build` for `dotnet publish`, then `FROM mcr.microsoft.com/dotnet/aspnet:9.0 AS runtime` and `COPY --from=build /app/publish .`. The `aspnet` runtime image is approximately 220 MB.

> [!warning] A missing or misplaced `global.json` allows the local SDK version to silently differ from CI
> Without `global.json`, `dotnet` uses the highest installed SDK on the current machine. A developer with .NET 9 and a CI runner with .NET 8 will compile different language versions, potentially producing unreproducible behavior or CI failures on valid local code.

> [!success] Correct pattern
> Place `global.json` at the repository root with `"rollForward": "latestPatch"` and the minimum patch version of the desired SDK. Pass `global-json-file: global.json` to `actions/setup-dotnet@v4` in CI so the same constraint applies in every environment.

> [!warning] `dotnet add package` without a version constraint installs the latest version, breaking lockfile stability
> Running `dotnet add package Dapper` without `--version` installs the current latest. On the next machine or CI run, `dotnet restore` without `--locked-mode` will resolve the same "latest" — which may be a different version by then.

> [!success] Correct pattern
> Always specify the version: `dotnet add package Dapper --version 2.1.28`. After adding, run `dotnet restore` to regenerate `packages.lock.json` and commit both files together.

## Recommendations

- **Place `global.json` at the repository root from day one.** Adding it later requires all team members to install the pinned SDK version simultaneously — do it before the first commit.
- **Use Central Package Management (`Directory.Packages.props`) for solutions with more than two projects.** It eliminates version drift across projects and makes upgrades a single-file change with a single PR.
- **Always use `dotnet publish` in Dockerfiles, never `dotnet build`.** `publish` produces the exact file set needed at runtime; `build` includes `.pdb` files, test projects, and intermediate artifacts that inflate the image.
- **Pin the `actions/setup-dotnet@v4` action version in CI.** Unpinned `@v4` will silently update when GitHub releases a new patch version of the action, which can change default behavior. Pin to the full SHA for critical pipelines.
- **Do not install dev tools (`dotnet-ef`, `coverlet.console`) in the production Docker image.** These belong in `dotnet tool install --local` in a `.config/dotnet-tools.json` file, which CI can restore separately from the runtime image.
- **Validate `dotnet restore --locked-mode` in CI before merging.** This catches unintended dependency upgrades introduced by a changed `<PackageReference>` without a corresponding lockfile update.
- **Use `mcr.microsoft.com/dotnet/aspnet` as the runtime base image for web apps** and `mcr.microsoft.com/dotnet/runtime` for console apps. The `aspnet` image includes ASP.NET Core middleware but not the full SDK.
- **Prefer `dotnet list package --outdated` in a weekly dependency review pipeline.** Keeping dependencies current in small increments is far cheaper than addressing a major-version upgrade backlog once a year.

## Troubleshooting

| Problem | Cause | Fix |
|---|---|---|
| `NETSDK1045: The current .NET SDK does not support targeting .NET X` | Installed SDK is older than the `TargetFramework` in `.csproj` | Install the required SDK version; add `global.json` to pin it going forward |
| `dotnet restore` fails with `Unable to load the service index for source` | NuGet feed URL unreachable; corporate proxy blocking nuget.org | Configure a proxy in `NuGet.Config` or add a corporate mirror; verify `dotnet nuget list source` shows the correct URL |
| `packages.lock.json` causes `NETSDK1004` in CI | `RestoreLockedMode` is enabled but the lockfile is out of date after a `.csproj` change | Run `dotnet restore` locally to regenerate the lockfile, then commit the updated file alongside the `.csproj` change |
| Docker build fails with `COPY --from=build /app/publish .` — directory not found | `dotnet publish` output path in the build stage does not match the `COPY` source path | Add `-o /app/publish` to the `dotnet publish` command to explicitly set the output directory |
| `CS8652: The feature 'X' is currently in Preview` in CI but not locally | CI uses a different (older) SDK version than the developer's machine | Add `global.json` with the developer's SDK version; set `--locked-mode` in CI's `dotnet restore` step |
| NuGet package not found in CI but works locally | Package exists on a private feed not configured in the CI environment | Add the private feed credentials as repository secrets; configure them with `dotnet nuget add source` in the CI setup step |
| `dotnet tool restore` fails after a fresh clone | `.config/dotnet-tools.json` references a tool not in the configured NuGet feeds | Ensure the private feed is configured before `dotnet tool restore`; add any private tool feeds to `NuGet.Config` |
| Cloud Run deployment fails with `No such file or directory: /workspace` | `gcloud builds submit` Dockerfile context path is wrong or `.dockerignore` excludes needed files | Verify the Dockerfile `COPY` paths against the actual project layout; check `.dockerignore` is not excluding `*.csproj` or `*.sln` |
