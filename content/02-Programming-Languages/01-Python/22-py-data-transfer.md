...
Copies test files and credentials to the VM, then runs the upload benchmark script remotely via SSH.
...
Side-by-side throughput comparison between local and VM origins, with interactive chart.
...
Three methods for downloading GCS blobs to local disk, mirroring the top-ranked upload counterparts.
...
Three methods for pulling files from the GCP VM to the local machine over SSH.
...
Each step in the end-to-end pipeline: compress, split, parallel upload, parallel download, verify, and merge.
...
>
> Using `.upload_from_filename()` without the Transfer Manager caps throughput at ~40–80 MB/s on a typical VM regardless of the available bandwidth, because the GCS client sends one chunk at a time over a single connection.

> [!success] Correct pattern
>
> Use `transfer_manager.upload_many_from_filenames()` or `transfer_manager.upload_chunks_concurrently()` with `max_workers` set to at least the number of available vCPUs. For a file larger than 1 GB, `upload_chunks_concurrently` with `chunk_size=32 * 1024 * 1024` and `worker_type=PROCESS` achieves near-linear scaling up to 8 workers on a n2-standard-8 VM.

> [!warning] `subprocess`-based transfers can fail silently
>
> `subprocess.run(["rsync", ...])` returns successfully (exit code 0) even when `rsync` encounters partial failures on individual files if `--ignore-errors` is set. Checking only that the process did not raise an exception is not sufficient.

> [!success] Correct pattern
>
> Always check `result.returncode == 0` and capture `result.stderr`. For `gcloud storage cp`, also validate the object count and size against the source after the call completes.

> [!warning] `BULK INSERT` reads from the SQL Server host filesystem, not the client
>
> Specifying a local client path (e.g., `C:\Users\me\data.csv`) in a `BULK INSERT` statement causes a server-side file-not-found error because SQL Server resolves the path on the database host, not the machine running the Python script.

> [!success] Correct pattern
>
> Copy the CSV to a UNC share or a path accessible to the SQL Server service account before issuing `BULK INSERT`, or use `pandas.to_sql()` with `fast_executemany=True` when the file must stay on the client machine.

> [!warning] BigQuery streaming insert (insertAll) is not a bulk-load substitute
>
> `client.insert_rows_json()` sends each batch over HTTP and is billed per byte inserted — at 1 GB+ scale this is both slow (hundreds of API calls) and expensive compared to a load job reading directly from GCS.
...
- Pre-compress files with zstd (`level=3`) before uploading to GCS when the destination will be read repeatedly — the decompression overhead on read is lower than the storage and egress savings at scale.
- For SQL Server bulk inserts via `pandas.to_sql()`, always pass `chunksize=10_000` and `fast_executemany=True` on the SQLAlchemy engine; the default single-transaction path can exhaust `tempdb` on tables wider than ~50 columns.
- Pin `google-cloud-storage >= 2.14` to get Transfer Manager `upload_chunks_concurrently` — earlier versions require manual chunk splitting with `blob.compose()`.
- When running on a Compute Engine VM, use the metadata server (`http://metadata.google.internal`) to obtain credentials rather than loading a service-account key file — key files on disk are a secrets-management risk and unnecessary on GCE.
