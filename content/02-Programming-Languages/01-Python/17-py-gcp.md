...
>
> API keys, service account JSON files, and database passwords in source control are the #1 cause of cloud security breaches.
...
- **Use ADC (Application Default Credentials)** — zero-config in GCP; `gcloud auth application-default login` for local dev.
- **Use least-privilege IAM roles** — `roles/bigquery.dataEditor` instead of `Owner`. One role per service.
- **Use partitioned BigQuery tables** — partition by date to limit scan scope and reduce cost.
- **Use Secret Manager for all secrets** — versioned, audited, IAM-protected. Access at runtime, not import time.
- **Use structured logging** — `google.cloud.logging` with JSON payloads for queryability and alerting.
- **Use batch writes for Firestore** — `db.batch()` groups up to 500 writes in a single atomic operation.
- **Design Pub/Sub consumers to be idempotent** — at-least-once delivery means duplicates are expected.

## Troubleshooting

| Problem | Cause | Fix |
|---|---|---|
| `DefaultCredentialsError` | No ADC found — no env var, no gcloud auth, no metadata server | Run `gcloud auth application-default login` or set `GOOGLE_APPLICATION_CREDENTIALS` |
| BigQuery query unexpectedly expensive | `SELECT *` or unpartitioned table scanned fully | Select only needed columns; use partitioned/clustered tables |
| Pub/Sub subscriber receives duplicate messages | At-least-once delivery semantics | Implement idempotent processing with deduplication |
| Firestore write exceeds 1MB limit | Document too large | Restructure into subcollections or use GCS for large blobs |
