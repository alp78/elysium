...
> [!warning] if / elif / else — indentation-scoped conditional branching
...
> [!warning] Ternary expression — value_if_true if condition else value_if_false
...
> [!warning] Truthy/falsy — empty collections, 0, None, "" are falsy
...
> [!warning] match/case — structural pattern matching (Python 3.10+)
...
> [!warning] match with destructuring — bind variables from tuple/list/dict shapes
...
> [!warning] match with type checking — case int(n), case str(s)
...
> [!warning] for and while loops — Python's two loop constructs
...
> [!warning] range() — lazy integer sequence with start, stop, step
...
> [!warning] Iterating strings and dicts — characters and key-value pairs
...
> [!warning] enumerate and zip — indexed iteration and parallel pairing
...
> [!warning] while and for/else — condition loops and search pattern
...
> [!warning] do-while workaround and nested loops
> do-while workaround and nested loops
>
...
> [!warning] break, continue, pass — loop control statements
...
> [!warning] continue and pass — skip current iteration or do nothing
...
> [!warning] break only exits the innermost loop
> break only exits the innermost loop
>
...
> [!warning] Breaking outer loops — flag or extract-to-function
...
> [!warning] Walrus operator := — assign and test in one expression
...
> [!warning] Generator functions — yield produces values lazily on demand
...
> [!warning] Manual iteration with next() — advance a generator one step
...
> [!warning] List vs generator expression — eager vs lazy evaluation
...
> [!warning] yield from — delegate to a sub-generator or iterable
...
> [!warning] Infinite generator — while True + yield for unbounded sequences
...
> [!warning]- itertools — standard library for combinatorial and infinite iterators
...
> repeat:         {list(repeat('x', 3))}
> accumulate:     {list(accumulate([1,2,3,4]))}
...
> [!warning] Iterator protocol — __iter__ and __next__ for custom iterables
...
> [!warning] Practical flatten alternatives — choose by depth and dependencies
...
> [!warning] chain.from_iterable — flatten exactly one level of nesting
...
> [!warning] Iterative flatten with stack — no recursion depth limits
...
> [!warning] pd.json_normalize — flatten nested dicts into a DataFrame
...
> [!warning]- Flatten summary — decision guide by depth and data type
...
> [!warning] List comprehension — [expr for item in iterable if condition]
...
> [!warning] Nested comprehension — flatten and Cartesian product
...
> [!warning] Dict and set comprehensions — {key: val} and {val} syntax
...
> [!warning] map and filter — functional-style transformation and selection
...
> [!warning] reduce and built-in aggregations — fold into a single value
...
> [!warning] Sorting with key functions — sorted() and list.sort()
...
> [!warning]- Summary — choosing the right iteration construct
