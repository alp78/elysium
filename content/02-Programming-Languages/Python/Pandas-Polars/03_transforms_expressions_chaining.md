...
---
## 2 · `assign` — Chainable Column Creation (Pandas)
...
---
## 6 · `map_elements` / `map_batches` (Polars)
...
---
## 7 · Conditional Columns — `np.where` / `np.select` (Pandas)
...
<small>shape: (5, 2)</small><table border="1" class="dataframe"><thead><tr><th>symbol</th><th>clean</th></tr><tr><td>str</td><td>str</td></tr></thead><tbody><tr><td>&quot;ENI.MI&quot;</td><td>&quot;ENI.MI&quot;</td></tr><tr><td>&quot;MUV2.DE&quot;</td><td>&quot;MUV2&quot;</td></tr><tr><td>&quot;CS.PA&quot;</td><td>&quot;CS.PA&quot;</td></tr><tr><td>&quot;BAS.DE&quot;</td><td>&quot;BAS&quot;</td></tr><tr><td>&quot;BBVA.MC&quot;</td><td>&quot;BBVA.MC&quot;</td></tr></tbody></table></div>



---
## 11 · `.dt` Accessor Operations
...
---
## 14 · Transforming `scores_daily` — Practical Examples
...
```python
ohlcv.select("symbol", "date", "close").head(5)
```
...
```python
ohlcv.select(pl.col("symbol", "date", "close")).head(3)
```
...
```python
# Regex
ohlcv.select(pl.col("^(open|high|low|close)$")).head(3)
```
...
```python
ohlcv.select(pl.exclude("id", "dividends", "stock_splits", "is_filled")).head(3)
```
...
```python
ohlcv.select("symbol", "date", pl.lit("EUR").alias("currency"), pl.lit(1.0).alias("weight")).head(3)
```
...
```python
ohlcv.select(pl.first("symbol"), pl.first("date").alias("first_date"), pl.last("date").alias("last_date"))
```
...
```python
ohlcv.filter(pl.col("symbol").is_in(["ASML.AS", "MC.PA"])).with_columns(
...
```python
ohlcv.filter(pl.col("symbol") == "ASML.AS").sort("date").with_columns(
...
```python
import polars.selectors as cs
scores.select(cs.numeric()).head(3)
```
...
```python
scores.select(cs.by_name("symbol", "score_date") | cs.float()).head(3)
```
...
```python
scores.select(cs.contains("score")).head(3)
```
...
- **Query**: Filter rows using a string expression (Pandas).
- **Sort**: Reorder rows by column values (Pandas).
- **Pipe**: Pass DataFrame through a function. Enables clean method chaining.
