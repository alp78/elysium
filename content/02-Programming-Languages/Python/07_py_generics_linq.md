...
```python
# TypeVar — declare a generic type variable for type hints; not enforced at runtime
T = TypeVar("T")
...
```python
# GroupBy with defaultdict — most common Python pattern for grouping
...
# itertools.groupby — requires pre-sorted data
...
```python
# Join — build a lookup dict for O(1) matching, then combine with comprehension
...
inner_join = [
    ...
    for emp in employees
    if emp["dept"] in dept_lookup
]
print("Inner join:")
for r in inner_join[:3]:
    print(f"  {r['name']:10} {r['dept']:15} head={r['head']:10} budget=${r['budget']:,}")

# Left join — all employees, departments may be missing
print("\nLeft join:")
for emp in employees[:3]:
    dept = dept_lookup.get(emp["dept"], {})
...
<h4>Chained transformations and <code style="font-size:0.75em">zip</code></h4>

```python
# Chained transformations — filter → transform → sort → take in one expression
result = sorted(
    [
...
## Pandas vs LINQ — side-by-side comparison

#### DataFrame creation and filtering

```python
# DataFrame creation and filtering — boolean indexing for Where-like operations
df = pd.DataFrame([
    {"name": "Alice", "dept": "Engineering", "salary": 95000, "level": "senior"},
    {"name": "Bob", "dept": "Sales", "salary": 65000, "level": "junior"},
    {"name": "Charlie", "dept": "Engineering", "salary": 110000, "level": "lead"},
...
<h4>Select, Sort, and <code style="font-size:0.75em">GroupBy</code></h4>

```python
# Select columns, add computed column, sort, and groupby with aggregations

# Select columns
print("Select: name + salary")
print(df[["name", "salary"]])

# Add computed column with assign
...
<h4><code style="font-size:0.75em">pd.merge</code> and chained pipeline</h4>

```python
# pd.merge for joins; method chaining for full pipeline

# Join — pd.merge combines DataFrames by key
...
# Chained pipeline — filter → transform → sort → take
print("\nChained pipeline:")
result = (
    df[df["salary"] > 75000]
