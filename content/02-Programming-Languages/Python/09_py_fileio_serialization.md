...
> [!warning] File I/O — open(), modes, encoding, and the with statement
...
> [!warning] CSV files — the standard tabular interchange format
...
> [!warning] json.dumps — serialize dict to JSON string
...
> [!info] JSONL is the standard format for:
...
> [!warning] yaml.safe_load — parse YAML string into Python dict
...
> [!warning] Serialization — converting objects to bytes/string for storage or transfer
...
> [!example] struct converts Python values ↔ C-compatible binary data
...
> [!warning] Character encoding — UTF-8, ASCII, Latin-1 conversion
...
> [!warning] aiofiles — async file I/O for concurrent Python applications
...
> [!warning] orjson — high-performance JSON serialization (3-10x faster than json)
...
> [!warning] Pydantic — typed models with automatic validation from JSON/dict
...
> [!warning] Polars and DuckDB — high-performance CSV parsing (10-100x faster)
...
> [!warning] fsspec / smart_open — unified filesystem for local + cloud storage
