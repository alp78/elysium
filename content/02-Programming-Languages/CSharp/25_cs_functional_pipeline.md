...
function loadDotnetInteractiveApi() {
    ...
    probeAddresses(["http://2a02:8308:718a:f200::655c:2048/","http://2a02:8308:718a:f200:8bd4:d06d:33ed:be05:2048/","http://2a02:8308:718a:f200:259e:c6fe:ecc9:c7d0:2048/","http://2a02:8308:718a:f200:6845:a19f:d23f:cd1:2048/","http://2a02:8308:718a:f200:b054:a876:b2c0:1ae5:2048/","http://fe80::3212:d8da:d32d:4723%14:2048/","http://192.168.0.110:2048/","http://::1:2048/","http://127.0.0.1:2048/","http://fe80::f30b:73d:48d0:8d99%45:2048/","http://172.25.64.1:2048/"])
        ...
        context: '39800.Microsoft.DotNet.Interactive.Http.HttpPort',
...
<div><div></div><div></div><div><strong>Installed Packages</strong><ul><li><span>Dapper, 2.1.72</span></li><li><span>FluentValidation, 12.1.1</span></li><li><span>Microsoft.Data.SqlClient, 7.0.0</span></li><li><span>ParquetSharp, 21.0.0</span></li><li><span>Plotly.NET, 5.1.0</span></li><li><span>Plotly.NET.CSharp, 0.13.0</span></li><li><span>Plotly.NET.Interactive, 5.0.0</span></li><li><span>Polars.NET, 0.4.0</span></li><li><span>Polars.NET.Native.win-x64, 0.4.0</span></li><li><span>Polly, 8.6.6</span></li></ul></div></div>
...
- `RawOhlcv` — validates Yahoo Finance output at Bronze ingestion
...
The records above track **what happened** (row counts, timing, hashes).
The records below track **what the data means** — column semantics,
...
1. `StartStage()` — captures timestamp and input row count
2. (pipeline stage runs)
3. `EndStage()` — fills output metrics, computes SHA-256 hash
...
**`dim_symbol`** — company metadata from HttpClient with **SCD Type 2** historization.
...
**`dim_calendar`** — per-exchange trading calendar built from dim_calendar SQL query.
...
1. **Fetch** — download OHLCV from Yahoo Finance API via HttpClient to `landing/{symbol}.json`
2. **Validate** — parse each row through FluentValidation `RawOhlcvValidator`
...
DataTable in, DataTable out, no side effects. They can be unit-tested with
a hardcoded 10-row DataTable, no database or network required.
...
The serving layer (HttpListener API) reads Parquet files, not SQL Server. This is the
...
JSON files (serialized from DataTables).
...
Visual validation of the pipeline output using Plotly.NET with dark-theme
