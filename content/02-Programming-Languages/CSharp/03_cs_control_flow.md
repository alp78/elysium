...
```csharp
// if / else if / else — braces required for multi-statement blocks; conditions must be explicit bool expressions
#nullable enable
...
```csharp
// Nested ternary — chain of ? : for multiple conditions; reads right-to-left
int val = 15;
...
```csharp
// Collections, strings, and null don't auto-convert to bool
// every condition must be an explicit bool expression
...
```
    ...
    (14,7): warning CS8632: The annotation for nullable reference types should only be used in code within a '#nullable' annotations context.

#### Switch statement — discrete value matching with mandatory break

```csharp
// Switch statement — tests one value against multiple cases; break is required
...
```csharp
// Switch expression — uses => to return a value; 'or' combines cases; _ is the default wildcard
...
```csharp
// Relational patterns — >= and < directly in switch cases; first match wins so put most restrictive first
grade = score switch
{
...
```csharp
// Type patterns — match by runtime type and bind to a variable; when adds an additional guard condition
...
```csharp
// Property patterns — match an object by its property values using { PropertyName: value }
...
```csharp
// Null-coalescing — ?? returns left if not null, else right; ??= assigns only when null; is extracts type+value
...
```
    ...
    (2,7): warning CS8632: The annotation for nullable reference types should only be used in code within a '#nullable' annotations context.

## Loops

```csharp
// for and foreach loops — for uses (init; condition; increment); foreach iterates any IEnumerable<T>
...
```csharp
// break and continue — break exits the innermost loop; continue skips to the next iteration
...
```csharp
// goto breaks out of nested loops — C# has no labeled break; goto to a label after the loops is the idiom
bool found = false;
for (int i = 0; i < 3; i++)
{
    for (int j = 0; j < 3; j++)
...
```csharp
// yield return — pauses the method, returns a value, and resumes on the next iteration; compiler builds a state machine
...
```csharp
// GetEnumerator() + MoveNext() + Current — manual iteration protocol
IEnumerable<int> Countdown(int n)
{
...
```csharp
// SelectMany flattens exactly one level — does not recurse into deeper nesting
...
```csharp
// Iterative flatten using Stack<T> — avoids recursion; processes arbitrarily deep nesting without call stack growth
...
```csharp
// Eager vs lazy evaluation — ToList() forces immediate execution; without it, query runs on demand
...
```csharp
// yield break — stops the iterator early, equivalent to a return in a regular function
...
```csharp
// LINQ basics — querying and transforming collections with extension methods
...
```csharp
// Query syntax vs method syntax — both compile to identical IL; query syntax reads like SQL
var words = new[] { "hello", "world", "csharp", "is", "great" };
...
```csharp
// ToDictionary and ToHashSet — materialize LINQ results into specific collection types

// ToDictionary — build a dictionary from a sequence using key and value selectors
...
```csharp
// Aggregate and built-in aggregations — Aggregate is the general fold; Sum/Max/Min/Any/All are optimized shortcuts

// Aggregate — general reduce: starts with seed, applies func(accumulator, element) for each element
...
```csharp
// Ordering with a key selector — a lambda that extracts the value to compare (e.g. n => n.Length)
...
```csharp
// Deferred execution — nothing runs until .ToList() or foreach; each method returns a lazy IEnumerable
...
```csharp
// Infinite generator with yield return — produces values on demand; caller controls how many to take

// Naturals — infinite sequence starting from a given value; never terminates on its own
