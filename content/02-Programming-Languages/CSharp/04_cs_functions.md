...
> [!warning] Function basics — methods, return types, void, and parameters
...
> [!warning] Expression-bodied methods
...
> [!warning] Func<T,TResult> and Action<T> — store and pass function references
...
> [!warning] Callbacks — pass Action<T> for success/error response handling
...
> [!warning] Strategy pattern — swap behavior at runtime via Func parameters
...
> [!warning] Pipeline — chain Func steps sequentially with Aggregate
...
> [!warning] Dependency injection via Func — inject fake time for testability
...
> [!warning] Production call — default Func<DateTime> uses real UtcNow
...
> [!warning] Test with injected time — fixed DateTime for deterministic tests
...
> [!warning] Progress callback — optional Action for reporting iteration progress
...
> [!warning] onProgress?.Invoke(...) breakdown:
...
> [!warning] Sorting with Func key — OrderBy/OrderByDescending with lambdas
...
> [!warning]- Summary — common function-passing patterns reference
...
> [!warning] Default and named parameters — optional values and argument clarity
...
> [!warning] ref — pass by reference for two-way communication
...
> [!warning] out — method must assign a value before returning
...
> [!warning] in — read-only reference avoiding copies of large value types
...
> [!warning] params — accept any number of arguments as an array
...
> [!warning] No **kwargs — C# alternatives for flexible key-value arguments
...
> [!warning] Lambda expressions — anonymous inline functions
...
> [!warning] Statement lambda — multi-line with braces and explicit return
...
> [!warning] Lambdas with LINQ — inline functions for sorting, filtering, projection
...
> [!warning] Action, Predicate, and closure capture
...
> [!warning] Closure capture — lambda captures variable reference, not snapshot
...
> [!warning]- Summary — lambda vs named method decision guide
...
> [!warning] Block scope — variables are scoped to their enclosing braces
...
> [!warning] Closures — returned lambda keeps enclosing scope's variables alive
...
> [!warning] Closure modification — lambda and caller share the same variable
...
> [!warning] Counter factory — each call creates independent closure state
...
> [!warning] Range validator factory — parameterized closure for reusable checks
...
> [!warning] Loop capture gotcha — all lambdas share the same loop variable
...
> [!warning] Delegate types — type-safe function pointers
...
> [!warning] Multicast delegates — combine multiple handlers with +=
...
> [!warning] Method groups — pass a method name directly as a delegate
...
> [!warning] Callback via Action — per-item notification during processing
...
> [!warning] Method overloading — same name, different parameter signatures
...
> [!warning] Overload resolution — compiler picks best match by argument types
...
> [!warning] Extension methods — add methods to types you don't own
