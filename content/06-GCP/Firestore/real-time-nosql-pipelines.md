...
Precomputed features written asynchronously during batch jobs are read synchronously at inference time. Firestore delivers sub-10ms p99 on document reads with the right data model (one document per entity key). Related: [[querying-and-cost-optimization]] for BigQuery feature materialization patterns.
...
The following metrics are available under the `firestore.googleapis.com` namespace in [[cloud-monitoring-metrics|Cloud Monitoring]]:
...
Firestore does not log individual document reads/writes to [[cloud-logging|Cloud Logging]] by default. Enable Data Access audit logs to capture them:
...
For pure analytics with no real-time read requirement, [[data-loading-and-export|BigQuery streaming inserts]] are cheaper than Firestore writes. Use Firestore only when the real-time listener or low-latency lookup justifies the cost.
...
All server-to-server Firestore access should use dedicated service accounts — never user credentials or default compute service accounts in production. See [[service-accounts-and-iam]] for service account creation and key management.
...
1. Check `firestore.googleapis.com/api/request_latencies` in [[cloud-monitoring-metrics|Cloud Monitoring]] — filter by method `BatchWrite` or `Commit`.
...
1. Verify the write succeeded — check for exceptions in application logs via [[cloud-logging|Cloud Logging]].
