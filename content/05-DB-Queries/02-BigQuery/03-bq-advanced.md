...
|---|---|---|---|
| **`QUALIFY`** | BigQuery-exclusive clause that filters on window function results inline, without requiring a subquery wrapper. `SELECT ... QUALIFY ROW_NUMBER() OVER (...) = 1`. | Eliminates the subquery + WHERE pattern for deduplication and top-N-per-group. Not ANSI SQL, not available in SQL Server. | Using QUALIFY in dbt models that target multiple engines — it fails on SQL Server. Use the subquery pattern for cross-engine compatibility. |
| **`GENERATE_DATE_ARRAY`** | BigQuery function that produces an array of consecutive dates between two bounds. `UNNEST(GENERATE_DATE_ARRAY(...))` creates a date spine without recursion. | Preferred over recursive CTEs for date-series generation — no iteration limit, single-pass, more idiomatic BigQuery. | Using recursive CTEs for date series in BigQuery — it works but hits the 500-iteration default limit and is slower than GENERATE_DATE_ARRAY. |
| **`EXCEPT DISTINCT`** | BigQuery's explicit set-difference operator. Returns rows from the first query that do not appear in the second, after deduplication. SQL Server uses `EXCEPT` (implicit DISTINCT). | Same behavior as SQL Server's `EXCEPT`, but BigQuery requires the `DISTINCT` keyword explicitly. | Omitting `DISTINCT` — BigQuery syntax requires `EXCEPT DISTINCT` (not bare `EXCEPT`). |
...
- **Lateral join equivalents** — ROW_NUMBER pattern replacing SQL Server's CROSS APPLY/OUTER APPLY, complete grid generation
- **PIVOT / UNPIVOT** — native BigQuery PIVOT, portable CASE-based pivot, score component unpivot
- **MERGE (upsert)** — syntax, DML quota constraints, staging pattern
- **EXISTS / NOT EXISTS** — semi-joins, anti-joins
- **GROUPING SETS / ROLLUP / CUBE** — multi-level aggregation in one pass
- **String functions** — STRING_AGG, STRPOS, SUBSTR, REGEXP_EXTRACT
- **NULL handling** — COALESCE, IFNULL, NULLIF, SAFE_DIVIDE, IS NOT DISTINCT FROM
- **Set operations** — UNION ALL, UNION, INTERSECT, EXCEPT DISTINCT
- **Date & calendar patterns** — trading calendar, business day arithmetic, GENERATE_DATE_ARRAY
- **Temp tables vs CTEs** — decision guide, materialization cost trade-offs
