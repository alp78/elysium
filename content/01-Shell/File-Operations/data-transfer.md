...
Open an IAP tunnel to port 22 on the VM, then point rsync at the local tunnel endpoint. The tunnel runs in the background. For simpler one-off transfers, use `gcloud compute scp` instead. For IAP tunnel details, see [iap-tunneling](https://alp78.github.io/elysium/01-Shell/Networking/iap-tunneling).
